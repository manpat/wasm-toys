...
fn diff_cyclic(a: f32, b: f32, circ: f32) -> f32 {
	(b - a + 3.0*circ/2.0) % circ - circ/2.0
}
...
impl<'m> Chart<'m> {
	pub fn to_manifold(&self, p: Vec2) -> Option<Vec2> {
		...
		Some(vec2_map!(p, element.asin()/(2.0*PI)) * self.manifold.circumference)
	}
	...
	pub fn from_manifold(&self, p: Vec2) -> Option<Vec2> {
		...
		// [-1, 1]
		let x_diff = diff_cyclic(self.proj_point.x, p.x, circ.x) / circ.x * 2.0;
		let y_diff = diff_cyclic(self.proj_point.y, p.y, circ.y) / circ.y * 2.0;
